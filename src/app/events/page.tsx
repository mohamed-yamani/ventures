      ...
      <div className="container flex w-full flex-col items-start justify-center gap-4 pb-16">
        <div className="gap-2sm:grid-cols-2 grid w-full grid-cols-1 gap-2 pt-32 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3">
          {images.map(({ src, date, title, link }, index) => (
            <EventsCard src={src} date={date} title={title} key={index} />
          ))}
        </div>
      </div>
    </main>
...
function EventsCard({ src, date, title, key }: EventsCardProps) {
  return (
    <div
      key={key}
    ...
    >
      ...
      <div className="absolute translate-x-6 translate-y-6 rounded-none bg-secondary px-4 py-2 font-light text-primary text-white transition duration-300 ease-in-out">
